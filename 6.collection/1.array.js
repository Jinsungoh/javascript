...
let a = [1,2]
let b = [2,3]
let c = a.concat(b)//모든배열값을 return함
console.log(c)
...
console.log(arr)
console.log(arr.pop())//마지막값을 출력함.,statck
console.log(arr)
...
console.log(arr)
console.log(arr.shift())//앞의값을 출력함.
console.log(arr)
...
arr = [0, 7 ,8, 5]
arr.splice(0, 2, 1,2,3,4)//splice(0,x) x자리이후만큼 잘라낸다.
console.log(arr)
...
//sort(오름차순) 는 string으로 정리한다,유니코드 순서이다.
arr = [4, 3, 1, 2]
let s = arr.sort()
console.log(arr,s)
...
//과제: 위와 다른 for로 arr을 interating 하라.
...
//callback 에서 true 가나오는 e을 리턴한다.,중복일경우 처음발견한것만 나옴
const items = [
